...
const onSubmit = (step: number) => {
  order.orderType = typeTabs[state.activeTab].text
...
<template>
  <div class="form-main">
    ...
    <main class="form">
      <el-form :inline="true" :model="order" class="infoForm">
        <el-form-item :label="state.activeTab===2? '生鲜类型*': '物品类型*'">
          <el-input v-model="order.type" @blur="computePrice"/>
        </el-form-item>
        <el-form-item label="付款方式*">
          <el-select v-model="order.payment" placeholder="请选择">
        ...
        <el-form-item label="物品重量*">
          <el-input v-model="order.weight" @blur="computePrice"/>
        </el-form-item>
        <el-form-item label="保价">
          <el-input v-model="order.insurance" @blur="computePrice"/>
        </el-form-item>

        <el-form-item :label="'物品体积' + (state.activeTab===3? '*': '')">
          <el-input v-model="order.volume" @blur="computePrice"/>
        </el-form-item>
        <el-form-item label="增值服务">
          <el-select v-model="order.valueAdded" placeholder="可选">
        ...
        <el-form-item label="预计运费">
          <el-input disabled v-model="order.price"/>
        </el-form-item>
        <el-form-item label="时效选择*" v-if="state.activeTab===1">
          <el-select v-model="order.time" placeholder="请选择">
